...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        // Haptics addon code:
        // (reference: https://forum.unity.com/threads/haptic-feedback-on-different-devices.750509/)
        Gamepad gamepad;
        private float walkInterval = 0.5f;
        private float runInterval = 0.25f;
        ...
        private void Move()
        {
            ...
            else if(currentHorizontalSpeed > 0f && Grounded)
            {
                ...
                if(currInterval <= 0f)
                {
                    if(isRightFoot)
                    {
                        gamepad.SetMotorSpeeds(0f, stepIntensity);
                    }
                    else
                    {
                        gamepad.SetMotorSpeeds(stepIntensity, 0f);
                    }
                    isRightFoot = !isRightFoot;
                    if(_input.sprint)
                    {
                        currInterval += runInterval;
                    }
                    else
                    {
                        currInterval += walkInterval;
                    }
                }
            }
            else
